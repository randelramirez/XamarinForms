    ...
    public class NamedFontSizesPage : ContentPage
    {
        public NamedFontSizesPage()
        {
            ...
            foreach (NamedSize namedSize in namedSizes)
            {
                ...
                formattedString.Spans.Add(new Span { Text = String.Format("Named Size = {0} ({1:F2})", namedSize, fontSize), FontSize = fontSize });
                if (namedSize != namedSizes.Last())
                {
                    formattedString.Spans.Add(new Span { Text = Environment.NewLine + Environment.NewLine });
                }
            }
